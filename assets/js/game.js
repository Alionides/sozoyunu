...
const wordle = {
  todaysWord: possibilities[Math.floor(Math.random()*210)], //possibilities[Math.floor((new Date() - new Date(2022, 0, 0)) / 8.64e7)],
...
//process the keystrokes
  ...
  processKey(event) {
    ...
    if ("abcdefghijklmnopqrstuvwxyzəışçöüğ".includes(event.key)) {
      //ok it's a valid alpha input
      this.currentWordPosition = this.getCurrentWordPosition()
    ...
    if (possibilities.includes(_word)) {
      ...
      for (index = 0; index < 5; index++) {
        if (this.todaysWord.includes(_words[index])) {
          ...
          if (this.todaysWord.indexOf(_words[index]) == index || this.todaysWord == _word) {
